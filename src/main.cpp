...
void sendSensorReadings(int aqiIndex, float humidity_f, float temp_f, String sensorID) {
    Serial.println(sensorID);
    ...
    Serial.println(String("Bearer " + authToken).c_str());

    client.sendHeader("Accept", "application/json");
    client.sendHeader("Authorization", String("Bearer " + authToken).c_str());
